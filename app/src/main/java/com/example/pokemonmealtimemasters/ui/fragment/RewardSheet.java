...
/**
 * Bottom sheet that congratulates the user with a new Pokémon buddy.
 * Fetches the sprite and name from PokeAPI and displays them.
 */
public class RewardSheet extends BottomSheetDialogFragment {
    private static final String ARG_BUDDY = "buddy_key";
    private String buddyKey;

    public static RewardSheet newInstance(String buddyKey) {
        RewardSheet sheet = new RewardSheet();
        Bundle args = new Bundle();
        args.putString(ARG_BUDDY, buddyKey);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        if (getArguments() != null) {
            buddyKey = getArguments().getString(ARG_BUDDY);
        }
    ...
    @Override public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        title.setText(R.string.reward_title);

        // Set up Retrofit pointing at PokeAPI:
        ...
        api.getPokemon(buddyKey.toLowerCase()).enqueue(new Callback<>() {
            @Override
            public void onResponse(@NonNull Call<PokeApiService.Pokemon> call,
                                   @NonNull Response<PokeApiService.Pokemon> resp) {
                if (!resp.isSuccessful() || resp.body() == null) {
                    title.setText("You caught " + buddyKey + "!");
                    return;
                }
                // capitalize first letter
                String name = resp.body().name;
                String disp = Character.toUpperCase(name.charAt(0)) + name.substring(1);
                title.setText("You caught " + disp + "!");

                // load sprite URL with Coil
                String spriteUrl = resp.body().sprites.front_default;
            ...
            @Override
            public void onFailure(@NonNull Call<PokeApiService.Pokemon> call, @NonNull Throwable t) {
                title.setText("You caught " + buddyKey + "!");
            }
        });
